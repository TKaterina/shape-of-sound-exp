...
expName = 'Audiogram'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\ktamp\\OneDrive\\Desktop\\experiment\\Audiogram_lastrun.py',
...
text_instr = visual.TextStim(win=win, name='text_instr',
    text='Hello!\n\nPlease press the spacebar when you hear a beep.\n\nPress spacebar to continue',
    font='Arial',
    pos=(0, 0), height=0.04, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=0.0);
InstructionsEnd = keyboard.Keyboard()
...
InstructionsEnd.keys = []
InstructionsEnd.rt = []
_InstructionsEnd_allKeys = []
# keep track of which components have finished
InstructionsComponents = [text_instr, InstructionsEnd]
...
while continueRoutine:
    ...
    # *InstructionsEnd* updates
    waitOnFlip = False
    if InstructionsEnd.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        InstructionsEnd.frameNStart = frameN  # exact frame index
        InstructionsEnd.tStart = t  # local t and not account for scr refresh
        InstructionsEnd.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(InstructionsEnd, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'InstructionsEnd.started')
        InstructionsEnd.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(InstructionsEnd.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(InstructionsEnd.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if InstructionsEnd.status == STARTED and not waitOnFlip:
        theseKeys = InstructionsEnd.getKeys(keyList=['space'], waitRelease=False)
        _InstructionsEnd_allKeys.extend(theseKeys)
        if len(_InstructionsEnd_allKeys):
            InstructionsEnd.keys = _InstructionsEnd_allKeys[-1].name  # just the last key pressed
            InstructionsEnd.rt = _InstructionsEnd_allKeys[-1].rt
...
# check responses
if InstructionsEnd.keys in ['', [], None]:  # No response was made
    InstructionsEnd.keys = None
thisExp.addData('InstructionsEnd.keys',InstructionsEnd.keys)
if InstructionsEnd.keys != None:  # we had a response
    thisExp.addData('InstructionsEnd.rt', InstructionsEnd.rt)
thisExp.nextEntry()
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('audiogram.xlsx'),
...
for thisTrial in trials:
    ...
    data = np.sin(2*np.pi*carrier*time)
    
    data = np.vstack([data, data]).T
    wave = sound.Sound(data, 
                       sampleRate=48000,
                       volume=volume)
    wave.play()
    key_resp.keys = []
